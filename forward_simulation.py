...
nx = 751
dx = 4.0
v = torch.from_file('./models/marmousi_vp.bin',
...
# source_locations
source_locations = torch.zeros(n_shots, n_sources_per_shot, 1,
...
# receiver_locations
receiver_locations = torch.zeros(n_shots, n_receivers_per_shot, 1,
                                 dtype=torch.long, device=device)
...
plt.tight_layout()
plt.savefig('./models/example_forward_model.jpg')

receiver_amplitudes.cpu().numpy().tofile('./models/marmousi_data.bin')
